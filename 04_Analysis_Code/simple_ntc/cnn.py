...
class CNNClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        for window_size, n_filter in zip(window_sizes, n_filters):
            cnn = nn.Conv2d(in_channels=1,
                            ...
                            kernel_size=(window_size, word_vec_dim) #kernel = filter
                            )
    ...
    def forward(self, x):
        ...
        if min_length > x.size(1): # window 사이즈 보다 짧은 문장이 들어와도 에러는 안나옴
        ...
        x = x.unsqueeze(1) # 채널이 1개이므로, 이미지 rgb같은경우는 3 , x.view(~)와 같ㄴ다.
